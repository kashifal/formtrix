...
generateRandomData(employeeNames, courseNames, employeeCourseCompletion) {
  ...
  return employeeNames.map(employeeName => {
    ...
    const data = courseNames.map(courseName => {  // Remove backslashes
      ...
      const completed = employeeCourseCompletion.has(employeeId) && employeeCourseCompletion.get(employeeId).get(courseId) === true;
