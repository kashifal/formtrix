...
class HeatMapChart extends Component {
        ...
        chart: {
          ...
          events: {
            dataPointSelection: (event, chartContext, config) => {
              const { employeeId, x: courseShortname } = config.w.config.series[config.seriesIndex].data[config.dataPointIndex];
  ...
  loadData() {
    ...
    axios.get(`https://glowing-paradise-cfe00f2697.strapiapp.com/api/employees/?populate=*&filters[company][id][$eq]=${companyId}`)
      .then(response => {
        ...
        ]).then(([courseNames, employeeCourseCompletion]) => {
  ...
  loadCourseNames() {
    return axios.get("https://glowing-paradise-cfe00f2697.strapiapp.com/api/courses/?populate=*")
      .then(response => response.data.data.map(course => ({
  ...
  loadEmployeeCourseCompletion(employeeIds) {
    const promises = employeeIds.map(employeeId =>
      axios.get(`https://glowing-paradise-cfe00f2697.strapiapp.com/api/employee-courses?filters[employee][id][$eq]=${employeeId}&populate=course`)
        .then(response => {
