  ...
  useEffect(() => {
    const fetchCompanyData = async () => {
      try {
        ...
        const response = await fetch(`https://glowing-paradise-cfe00f2697.strapiapp.com/api/companies?filters[id][$eq]=${companyId}&populate=*`);
        ...
        if (companyId) {
          try {
            const employeeResponse = await fetch(`https://glowing-paradise-cfe00f2697.strapiapp.com/api/employees?filters[company][id][$eq]=${companyId}&populate=*`);
  ...
  return (
    <Grid container spacing={gridSpacing}>
      ...
      <Grid item lg={6} md={6} sm={12} xs={12}>
        ...
        {showTodoList ? <ToDoList isLoading={isLoading} /> : isMonksTraining ? <InductionChart isLoading={isLoading} /> : <InductionChartCompany isLoading={isLoading} />}
      </Grid>
