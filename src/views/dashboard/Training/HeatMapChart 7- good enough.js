...
class HeatMapChart extends Component {
    ...
    loadData() {
        ...
        axios
            ...
            .then(([employeeNames, employeeIds, employeeCompanies, courseNames, employeeCourseCompletion]) => {
                const series = this.generateHeatmapData(
                    employeeNames,
                    employeeIds,
                    courseNames,
                    employeeCourseCompletion,
                    employeeCompanies
                );
    ...
    loadEmployeeCourseCompletion(employeeIds) {
        const promises = employeeIds.map((employeeId) => {
            return axios
                .get(
                    `https://glowing-paradise-cfe00f2697.strapiapp.com/api/employee-courses?filters[employee][id][$eq]=${employeeId}&populate[course]=shortname,datecompleted`
                )
                .then((response) => {
