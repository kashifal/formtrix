      ...
      companyNames.map(async (companyName) => {
        const data = await Promise.all(
          skillNames.map(async (skillName) => {
            const employees = await this.fetchEmployeesByCompanyAndSkill(
              companyName,
              skillName
            );
            ...
            const completedPercentage = completedCourses.reduce((sum, completed) => sum + completed, 0) / (employees.length * totalCourses) * 100;
