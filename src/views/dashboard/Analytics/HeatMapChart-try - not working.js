...
const heatmapSeries = companiesData.map((company) => {
  ...
  const companyData = companyEmployees.flatMap((employee) => {
    const completedCoursesIds = employee.attributes.skills.data.flatMap(
      (skill) => skill.attributes.courses.data.map((course) => course.id)
    );
...
return (
<div style={{ width: '100%', height: '800px' }}> <Chart options={chartOptions} series={heatmapData} type="heatmap" height="100%" /> </div>
);
};
