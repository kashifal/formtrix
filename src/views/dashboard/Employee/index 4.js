...
const Employee = () => {
  ...
  useEffect(() => {
    console.log('Params:', { id, courseId }); // Add logging to verify params
    const fetchData = async () => {
      try {
        ...
        console.log('Employee Courses Data:', employeeCoursesData.data);
  ...
  const selectedCourse = employeeCourses.find((course) => {
    console.log('Course Shortname:', course.attributes.course.data.attributes.shortname.trim(), 'Course ID from URL:', courseId);
    return course.attributes.course.data.attributes.shortname.trim() === courseId;
  });
  ...
  const expiryDate = calculateExpiryDate(
    completionDate,
    selectedCourse?.attributes?.course?.data?.attributes?.YearsExpire
  );
        ...
        <Grid item xs={12}>
          {selectedCourse ? (
                      ...
                      secondary={
                        <>
                          <Typography component="span" variant="body2">
                            {completionDate ? `Completed on ${formatDate(completionDate)}` : 'Not yet completed'}
                          </Typography>
                          <br />
                          <Typography component="span" variant="body2">
            ...
            <Card>
              <CardContent>
                <Typography variant="h5" component="div">
                  No Course Selected for {fullname}
                </Typography>
              </CardContent>
            </Card>
          )}
