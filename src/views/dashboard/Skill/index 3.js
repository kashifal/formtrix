        ...
        const fetchData = async () => {
            try {
                const [employeeResponse, courseResponse, employeeCoursesResponse] = await Promise.all([
                    ...
                    fetch(
                        `https://glowing-paradise-cfe00f2697.strapiapp.com/api/employee-courses?filters[employee][id][$eq]=${employee}&filters[course][shortname][$eq]=${course}&populate[course]=name,shortname,datecompleted`
                    ),
                    fetch(
                ...
                setCourseCompleted(courseData.data.length > 0 ? courseData.data[0].attributes : null);
                    ...
                    <Card>
                        <CardContent>
                            ...
                            <Typography variant="body2">
                                Status:{' '}
                                {courseCompleted ? `Completed on: ${formatDate(courseCompleted.DateCompleted)}` : 'Not completed yet'}
                            </Typography>
                        </CardContent>
                    </Card>
                </Grid>
