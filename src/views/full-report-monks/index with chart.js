...
const staffCourseData = employeesWithCourseCompletion.reduce((acc, employee) => {
                                    ...
                                    {skill.attributes.courses.data.length > 0 ? (
                                        skill.attributes.courses.data.map((course) => (
                                            <Typography key={course.id} variant="body2" sx={{ ml: 4 }}>
                                                Course: {course.attributes.name} - {course.completed ? <Chip label="Completed" color="success" size="small" /> : <Chip label="Not Completed" color="error" size="small" />}
                                            </Typography>
                                        ))
                                    ) : (
  ...
  {/* Chart Container - Place the Chart component inside */}
  <div id="staff-courses-chart">
            <Chart 
                options={chartOptions}  // Make sure you've prepared this
                series={chartData.series} // Make sure you've prepared this 
                type="bar" 
