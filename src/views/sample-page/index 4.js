        ...
        events: {
          dataPointSelection: (event, chartContext, config) => {
            ...
            const selectedEmployee = employees.find(employee =>
              employee.attributes.skills.data.some(skill =>
                skill.attributes.role === selectedSkill &&
                skill.attributes.courses.data.some(course =>
                  course.attributes.shortname === selectedCourse
                )
              )
            );
            ...
            if (selectedEmployee) {
              ...
              const skillId = selectedEmployee.attributes.skills.data.find(skill => skill.attributes.role === selectedSkill).id;
  ...
  useEffect(() => {
    if (selectedCompany) {
      const fetchEmployeesAndCourses = async () => {
        try {
          const response = await axios.get(`https://glowing-paradise-cfe00f2697.strapiapp.com/api/employees?filters[company][name][$eq]=${selectedCompany}&populate=skills.courses,company`);
