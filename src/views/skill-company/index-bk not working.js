    ...
    employeesData.forEach(employee => {
      employee.attributes.skills.data.forEach(skill => {
        skill.attributes.courses.data.forEach(course => {
          ...
          courseData.total += 1;
          if (course.attributes.completed) { // Ensure to access 'completed' correctly
      ...
      <FormControl fullWidth>
        <Select value={selectedCompany} onChange={handleCompanyChange} displayEmpty inputProps={{ 'aria-label': 'Without label' }}>
          ...
          {companies.map((company) => (
            <MenuItem key={company.id} value={company.attributes.name}>{company.attributes.name}</MenuItem>
          ))}
        </Select>
      </FormControl>
      <Box sx={{ mt: 3 }}>
                ...
                {employee.attributes.skills.data.length > 0 ? (
                  employee.attributes.skills.data.map((skill) => (
                    <Box key={skill.id} sx={{ ml: 2, mt: 1 }}>
                      <Chip label={`Skill: ${skill.attributes.role} - ${skill.attributes.description}`} color="primary" />
                      {skill.attributes.courses.data.length > 0 ? (
                        skill.attributes.courses.data.map((course) => (
                          <Typography key={course.id} variant="body2" sx={{ ml: 4, mt: 1 }}>
                            Course: {course.attributes.name} - {course.attributes.completed ? "Completed" : "Not Completed"}
                          </Typography>
                        ))
                      ) : (
                        <Typography variant="body2" sx={{ ml: 4, mt: 1 }}>No courses listed for this skill.</Typography>
                      )}
                    </Box>
                  ))
                ) : (
                  <Typography variant="body2" sx={{ ml: 2, mt: 1 }}>No skills listed.</Typography>
                )}
              </Box>
            ))}
          </React.Fragment>
